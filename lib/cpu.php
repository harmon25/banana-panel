...
class CPU {
    ...
    public static function cpu1() {

        $result = array();

        $getLoad = sys_getloadavg();
        $cpuCurFreq = round(file_get_contents("/sys/devices/system/cpu/cpu1/cpufreq/scaling_cur_freq") / 1000) . "MHz";
        $cpuMinFreq = round(file_get_contents("/sys/devices/system/cpu/cpu1/cpufreq/scaling_min_freq") / 1000) . "MHz";
        $cpuMaxFreq = round(file_get_contents("/sys/devices/system/cpu/cpu1/cpufreq/scaling_max_freq") / 1000) . "MHz";
        $cpuFreqGovernor = file_get_contents("/sys/devices/system/cpu/cpu1/cpufreq/scaling_governor");
    ...
    public static function heat() {
        ...
        $fh = fopen("/sys/devices/platform/sunxi-i2c.0/i2c-0/0-0034/temp1_input", 'r');
